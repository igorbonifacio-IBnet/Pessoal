#!/usr/bin/env python3
"""Gera dashboard.html com filtro por mês e categorização — Finanças Igor & Nath"""
...
CAT_KW = {
    'Alimentação': ['atacadão','bretas','assaí','banana','carne','mc donalds','mc donald','pizzaria',
                    'almoço','espetinho','padoca','panif','armazem','pamonharia','bem fast','mourão',
                    'açaí','ice coco','biscoito','abast carrefour','dia a dia','supermercado',
                    'compras nath','banana semáforo','abastec','pão','mercado','hortifruti'],
    'Transporte':  ['uber','99 pop','baratão abast','óleo hb20','troca de óleo','lavagem hb20',
                    'porto pneus','abastecimento','combustível','hb20','lavagem'],
    'Saúde':       ['drogasil','drogaria','mounjaro','vacina','plano de saúde','serasa','barra prot',
                    'médico','farmácia','remédio'],
    'Pet':         ['petshop','petz','biscoito bento','bento','pet','ração'],
    'Vestuário':   ['marisa','roupa melinda','vestido nath','sutiã','vivara','roupa','calçado',
                    'tênis','blusa','calça'],
    'Casa':        ['montagem','guarda roupa','papel de parede','madeira guarda','colchão igor',
                    'limpeza sofá','carrinho melinda','ventilador melinda','fita isolante',
                    'ferragi','módulo','luciano vieira','móveis','sofá','cama'],
    'Lazer':       ['academia igor','presente martin','milhas','açaí villa mix','cinema','show',
                    'viagem','hotel','bar','restaurante'],
    'Eletrônicos': ['amazon ps5','shopee','paygo','celular','notebook','tv','computador'],
...
SKIP = {'fatura dezembro','fatura janeiro','fatura fevereiro','fatura março','fatura abril',
        'fatura maio','fatura junho','fatura julho','fatura agosto','fatura setembro',
        'fatura outubro','fatura novembro','fatura','valor','total','descrição','data',''}
...
def pc(v):
    ...
    pts = s.split('.')
    if len(pts) > 2: s = ''.join(pts[:-1]) + '.' + pts[-1]
...
def extract(text):
    ...
    for row in rows:
        ...
        if c0 == 'GASTOS MENSAIS':     sec = 'g'; continue
        if c0 == 'RECEITA MENSAL':     sec = 'r'; continue
        if c0 == 'RESTAM PARA O MÊS':  sec = 's'; continue

        if sec == 'g':
            # ---- Inter (cols 6-9) ----
            di,dsc,vi,pi = g(row,6),g(row,7),g(row,8),g(row,9)
            if dsc and vi and dsc.lower() not in SKIP:
                v = pc(vi)
                if v > 0:
                    item = {'desc':dsc,'val':v,'parc':pi,'cat':cat(dsc)}
                    if di: item['date'] = di; d['inter_trans'].append(item)
                    else:  d['inter_inst'].append(item)
            if g(row,7).lower() == 'total' and g(row,8):
                d['total_inter'] = pc(g(row,8))

            # ---- Nubank (cols 12-14) ----
            nd,nv,np = g(row,12),g(row,13),g(row,14)
            if nd.lower() == 'total' and nv: d['total_nubank'] = pc(nv)
            elif nd and nv and nd.lower() not in SKIP:
                v = pc(nv)
                if v > 0: d['nubank'].append({'desc':nd,'val':v,'parc':np,'cat':cat(nd)})

            # ---- Caixa (cols 16-18) ----
            cd,cv,cp = g(row,16),g(row,17),g(row,18)
            if cd.lower() == 'total' and cv: d['total_caixa'] = pc(cv)
            elif cd and cv and cd.lower() not in SKIP:
                v = pc(cv)
                if v > 0: d['caixa'].append({'desc':cd,'val':v,'parc':cp,'cat':cat(cd)})

        if sec == 'g':
            if c0 == 'TOTAL' and c1: d['total_gastos'] = pc(c1)
            elif c0 and c0 not in ('DESCRIÇÃO',''):
                d['gastos'].append({'desc':c0,'val_str':c1,'val':pc(c1),
                                    'data':g(row,2),'obs':g(row,3),'status':g(row,4)})
        elif sec == 'r':
            if c0 == 'TOTAL' and c1: d['total_receitas'] = pc(c1)
            elif c0 and c0 not in ('DESCRIÇÃO',''):
                d['receitas'].append({'desc':c0,'val_str':c1,'val':pc(c1),
                                      'data':g(row,2),'status':g(row,4)})
        elif sec == 's':
            if   c0 == 'PARA PAGAR':   d['para_pagar']   = pc(c1)
            elif c0 == 'PARA RECEBER': d['para_receber']  = pc(c1)
            elif c0 == 'TOTAL':        d['saldo']         = pc(c1)

    if d['total_inter']  == 0: d['total_inter']  = round(sum(x['val'] for x in d['inter_inst']+d['inter_trans']),2)
...
.cartao-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px;margin-bottom:20px}
...
footer{text-align:center;color:#334155;font-size:.7rem;margin-top:20px;padding-top:10px;
...
<div class="box" style="margin-bottom:18px">
  <h2>Transações Inter (Detalhadas)</h2>
  <div class="table-wrap"><table>
...
<footer>Dashboard Financeiro · Igor &amp; Nath · Dados do Google Sheets</footer>
...
function getCatTotals(d){
  const t={};
  [...(d.inter_inst||[]),...(d.inter_trans||[]),...(d.nubank||[]),...(d.caixa||[])].forEach(x=>{
    t[x.cat]=(t[x.cat]||0)+x.val;
  });
  return t;
}
...
function mkCharts(d){
  ...
  const opts=(ax)=>({responsive:true,maintainAspectRatio:false,
    plugins:{legend:{display:false}},
    scales:{x:{ticks:{color:tc},grid:{color:gc}},y:{ticks:{color:tc,callback:brl},grid:{color:'#334155'}}}});
  ...
  CH.car=new Chart(document.getElementById('cCartoes'),{type:'bar',data:{
    ...
    datasets:[{data:[d.total_inter,d.total_nubank,d.total_caixa],borderRadius:6,
      backgroundColor:['#f59e0b88','#a855f788','#22c55e88'],
      borderColor:['#f59e0b','#a855f7','#22c55e'],borderWidth:2}]},
    options:opts()});
}
...
function render(d){
  ...
  // Cartão boxes
  document.getElementById('cartao-grid').innerHTML=`
    <div class="cartao-box"><h2>💳 Cartão Inter — Parcelas</h2>
      ${mkCards(d.inter_inst||[])}
      <div class="ctotal">Total fatura: ${fmt(d.total_inter)}</div></div>
    <div class="cartao-box"><h2>💳 Cartão Nubank</h2>
      ${mkCards(d.nubank||[])}
      <div class="ctotal">Total: ${fmt(d.total_nubank)}</div></div>
    <div class="cartao-box"><h2>💳 Cartão Caixa</h2>
      ${mkCards(d.caixa||[])}
      <div class="ctotal">Total: ${fmt(d.total_caixa)}</div></div>`;
  ...
  // Trans table
  document.getElementById('tbody-trans').innerHTML=(d.inter_trans||[]).map(r=>
...
switchMonth(ORDER.find(m=>DATA[m])||ORDER[0]);
</script>
</body>
</html>"""

def generate_html(all_data, updated):
    first = next((m for m in MONTH_SHEETS if all_data.get(m)), MONTH_SHEETS[0])
    btns = ''.join(
...
def main():
    ...
    for sheet in MONTH_SHEETS:
        ...
        all_data[sheet] = data
        if data:
            print(f'  OK  gastos={data["total_gastos"]:.0f}  receitas={data["total_receitas"]:.0f}')
        else:
            print(f'  Sem dados')
